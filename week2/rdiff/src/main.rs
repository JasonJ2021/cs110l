...
fn lcs(seq1: &Vec<String>, seq2: &Vec<String>) -> Grid {
    ...
    for i in 0..m {
        for j in 0..n {
            if seq1[i] == seq2[j]{
                let t = c.get(i, j).unwrap();
                c.set(i+1, j+1, t + 1).unwrap();
            }else{
                let t1 = c.get(i+1, j).unwrap();
                let t2 = c.get(i, j+1).unwrap();
                c.set(i+1, j+1, max(t1 ,t2)).unwrap();
            }
        }
    }
    return c;
...
fn main() {
    ...
    let x = read_file_lines(filename1).expect("file1 open failed!");
    let y = read_file_lines(filename2).expect("file2 open failed");
    let grid = lcs(&x, &y);
    print_diff(&grid, &x, &y, x.len(), y.len());
}
